...
export default function Page() {
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,_#fff_0%,_#fbf8f0_100%)] px-4 py-10 text-slate-900 md:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-10">
          ...
          <Card className="border-slate-200/80 bg-white/90 shadow-sm">
            ...
            <CardContent className="space-y-3 text-sm text-slate-600">
              {paymentMethods.map((method) => (
                <div key={method} className="rounded-2xl bg-slate-50 px-4 py-3">
                  {method}
                </div>
              ))}
              <div className="rounded-2xl border-2 border-dashed border-amber-300 bg-amber-50 p-4 text-center text-slate-500">
        ...
        <section className="grid gap-6 md:grid-cols-3">
          {membershipOptions.map((option) => (
            <Card key={option.code} className="border-slate-200/80 bg-white/90 shadow-sm">
              <CardHeader>
